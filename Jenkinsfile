      ...
      steps {     
        withPythonEnv('python3') {
         ...
         // sh script :"python3 -u techdebt_validation.py -m \"${env.ArtifactsFolder}/trigger_manifest.cache\" -d \"${env.ArtifactsFolder}/techdebt_data/\"", returnStatus: true, label: 'Run tech debt validation'
          script {
            try {
              sh "python3 -u techdebt_validation.py -m \"${env.ArtifactsFolder}/trigger_manifest.cache\" -d \"${env.ArtifactsFolder}/techdebt_data/\"", returnStatus: true, label: 'Run tech debt validation'"
            } catch (err) {
              echo err.getMessage()
