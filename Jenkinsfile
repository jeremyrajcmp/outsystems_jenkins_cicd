    ...
    stage('Validate Tech Debt') {
      steps {     
        withPythonEnv('python3') {
          ...
          def res = sh script :"python3 -u techdebt_validation.py -m \"${env.ArtifactsFolder}/trigger_manifest.cache\" -d \"${env.ArtifactsFolder}/techdebt_data/\"", returnStatus: true, label: 'Run tech debt validation'
          echo $res
        }        
      }
    }
